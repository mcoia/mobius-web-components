...
class BarcodeGenerator {
  ...
  private FPDFLabelMaker $labelmaker;
  ...
  protected $JStart = [
    ...
    "B" => 104,
    "C" => 105,
  ]; // Caractères de sélection de jeu au début du C128
  ...
  function __construct(FPDFLabelMaker $labelmaker) {
  ...
  function Generate($x, $y, $code, $w, $h): void {
    ...
    for ($i = 0; $i < strlen($crypt); $i++) {                                      // BOUCLE D'IMPRESSION
      ...
      for ($j = 0; $j < count($c); $j++) {
        $this->labelmaker->Rect($x, $y, $c[$j] * $modul, $h, "F");
        $x += ($c[$j++] + $c[$j]) * $modul;
      }
    }
  }
