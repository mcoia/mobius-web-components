jQuery(document).ready(function () {
  ...
  if (jQuery("#mobius_bnl_page_is_loaded").length > 0) {
    ...
    function checkGetable() {
      ...
      var good_dates = 0;
      var good_libs = 0;
      if (jQuery("#filter_switcher_word").html() != 'Advanced') // advanced filters
        // do not require any
        // library input
      {
        good_libs = 1;
    ...
    function bnlGetCookie(cname) {
      ...
      for (var i = 0; i < ca.length; i++) {
        ...
        if (c.indexOf(name) == 0) {
          return c.substring(name.length, c.length).replace(/v0v/g, ';').replace(/v1v/g, '&');
        }
      }
      return 0;
    }
    ...
    function bnl_get_seed_data(bnl_panel_dom, dropdown_vals) {
      ...
      // Figure out all the branches/clusters in the database
      jQuery.get("borrowing_n_lending_get?get_branches=1", function (ob) {
        ...
        if (branch_obj['branch'] && branch_obj['branch_order']) {
          for (var i in tarray) {
            var type = tarray[i];
            // var selectHTML = '<select multiple
            // id="'+type+'_'+dropdown_vals['branch']['chooser']+'"
            // data-placeholder="'+dropdown_vals['branch']['placeholder']+'(s)...">';
            ...
            for (var branch_pos in branch_obj['branch_order']) {
              // Make sure that this branch isn't one of those that needs to be
            ...
            if (type == 'owning') // Take care of the simple filter dropdown menu
            {
              ...
              jQuery('#' + type + '_' + dropdown_vals['branch_basic']['chooser']).chosen({max_selected_options: 1}).change(function () {
                checkGetable();
              });
            }
          ...
          // jQuery("#bnl_my_lending_div").html("<table
          // id='testing_table'><thead><tr><th>testing</th><th>testing2</th></tr></thead><tbody><tr><td>hello</td><td>hello2</td></tr></tbody></table>");
        ...
        if (branch_obj['cluster']) {
          for (var i in tarray) {
            ...
            for (var cluster in branch_obj['cluster']) {
              var t = {
                'name': branch_obj['cluster'][cluster]['name'],
                'id': cluster
              };
              clusterSort.push(t);
            }
      ...
      jQuery("#firsttimebutton").click(function () {
        bnl_firsttimeinstructions();
      });
      jQuery("#instructions_show_hide_a").click(function () {
        bnl_show_hide();
      });
    }
      ...
      jQuery.get("borrowing_n_lending_get?startdate=" + startDate + "&enddate=" + endDate + qstring, function (data) {
        ...
        for (var type in types) {
          if (data[type]) {
            ...
            // figure out the complete list of secondary libs for each plib
            // (cutting through all months)
            var slibs = {};
              ...
              for (var plib_pos in plibs) {
                ...
                for (var system_pos in systems) {
                  ...
                  for (var slib_pos in slibs[plib]) {
                    ...
                    {
                      if (branch_obj['branch_to_system'][slib] == system)// matching in order and
        ...
        // Finished building the "my" tables, Now onto the "Lib to lib"
        // table(s) - one for each month in range
          ...
          for (var plib_pos in lib_to_lib['months'][month]['down']) {
            ...
            if (selections['owning'].includes(plib) || selections['borrowing'].includes(plib)) // Make sure this lib was explicitly selected
            {
              ...
              for (var slib_pos in lib_to_lib['months'][month]['across']) {
                var slib = lib_to_lib['months'][month]['across'][slib_pos];
                var amount = 0;
                // Only running through the borrow data, This is essentially a
                // borrow diagram
        ...
        for (var month_pos in lib_to_lib['month_order']) {
          // Disabling this feature until we spend the time to code this to
          // reach out and flesh out all the rest of the data for the other
          // libraries to include on this table
          ...
          // bnl_wire("#"+lib_to_lib['months'][month]['table_dom'],[[0,'asc']],
          // 8); // column limit for screen realestate issues
        }
      });
    }
    ...
    function bnl_dedupe_array(a_array, inner = 0, lib_branch_order = 0) {
      ...
      for (var d in a_array) {
        var v = [d];
        if (inner) {
          if (typeof (a_array[d]) == 'object') // only going one more level and that's
            // enough please
          {
            v = [];
    ...
    function getMultiSelectOptions(dropdown_vals, empty_is_all = 1) {
      // This function creates an array of all of the libraries that are
      // included in the users selection Handling the non-selections as ALL
      var tarray = ["owning", "borrowing"];
      var ret =
        {
          owning: [],
      ...
      for (var i in tarray) {
        ...
        if (!somethingSpecified && empty_is_all) // All branches are included when no
          // clusters, no systems, no branches
          // are selected
        {
